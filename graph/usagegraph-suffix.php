...
{
	$sql = "select `suffix`, count(*) as NUM from ".$xoopsDB->prefix("usage")." where length(`suffix`)>0 group by `suffix` order by NUM limit 13";
} else {
	$sql = "select `suffix`, count(*) as NUM from ".$xoopsDB->prefix("usage_quarantine")." where length(`suffix`)>0 group by `suffix` order by NUM limit 13";
}
...
$title = array("Set" =>"Top 13 Suffix Usage", "SetColor" =>"green");
$legend =array("Pos" => array(0.07,0.48,"left","center"));
